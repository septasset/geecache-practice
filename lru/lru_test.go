...
func TestGet(t *testing.T) {
	lru := New(int64(0), nil)
	lru.Add("key1", String("a simple value"))
	if v, ok := lru.Get("key1"); !ok || string(v.(String)) != "a simple value" {
		t.Fatalf(`cache hit "key1"="a simple value" failed`)
	}
